...
async function releaseSerialConnection(reason: 'user' | 'lost' | 'error', error?: unknown): Promise<void> {
  ...
  if (reason === 'user') {
    ...
    errorMessage.value =
      'The serial device was lost. Reconnect the USB cable or reset the board, close other serial monitors, then connect again.';
...
function loadSample(): void {
  const sample = [
    ...
    'wifi,,19.4326080,-99.1332090,SampleNet,AA:BB:CC:DD:EE:FF,6,-65,WPA2_PSK',
